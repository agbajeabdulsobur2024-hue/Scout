...
def handle_update(update: dict) -> None:
    ...
    if text.startswith("/crimes"):
        send_message(chat_id, "🔍 Fetching MEXC Futures top 20 movers...")
        try:
            ...
            for t in raw:
                ...
                if not sym.endswith("_USDT"):
                    continue
                try:
                    rate = t.get("riseFallRate")
                    if rate is None:
                        continue
                    chg = round(float(rate) * 100, 2)
                    tickers.append({
                        "symbol":     sym.replace("_USDT", ""),
                        "change_pct": chg,
                        "volume":     float(t.get("volume24", 0) or 0),
            ...
            if not tickers:
                send_message(chat_id, "⚠️ MEXC data unavailable. Try again.")
                return
            ...
            gain_lines = ["📈 <b>TOP 20 GAINERS — MEXC Futures</b>\n"]
            for i, m in enumerate(gainers, 1):
                chg  = m["change_pct"]
                flag = "🚨" if chg > 30 else "⚠️" if chg > 10 else "📈"
                gain_lines.append(f"{flag} {i}. <b>{m['symbol']}</b>  {chg:+.2f}%")

            loss_lines = ["\n📉 <b>TOP 20 LOSERS — MEXC Futures</b>\n"]
